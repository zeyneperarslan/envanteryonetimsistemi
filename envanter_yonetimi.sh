...
function sifre_sifirla() {
    ...
    if zenity --question --title="Onay" --text="Şifreyi sıfırlamak istediğinizden emin misiniz?" --width=400 --height=150; then
        ...
        # Yeni şifreyi kontrol et (boş olamaz)
        if [[ -z "$yeni_sifre" ]]; then
            zenity --error --text="Yeni şifre boş olamaz!"
            return
        fi

        # Şifreyi güncelle
        sed -i "s/^$kullanici_adi,$soyadi,$rol,$mevcut_sifre$/$kullanici_adi,$soyadi,$rol,$yeni_sifre/" kullanici.csv
